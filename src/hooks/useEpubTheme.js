...
export const useEpubTheme = () => {
  const applyTheme = useCallback((rendition, mode, size, family) => {
    ...
    try {
      ...
      const captionColor = isDarkMode ? '#aaa' : isCreamMode ? '#8c7055' : '#666'
      const noteBorder = isDarkMode ? '#666' : isCreamMode ? '#b09070' : '#999'
      ...
      rendition.themes.register('reader-theme', {
        ':root': { ...zineVars, 'color-scheme': isDarkMode ? 'dark' : 'light' },
        ...
        '.note::before': { 'border-top-color': noteBorder },
        '.poem .note::before': { 'border-top-color': noteBorder },
        '.letter .note::before': { 'border-top-color': noteBorder },
        '.dialog .note::before': { 'border-top-color': noteBorder },
        'blockquote .note::before': { 'border-top-color': noteBorder },
        '.image-caption': { 'color': captionColor },
        '.info-box': { 'background-color': infoBoxBg + ' !important', 'border-color': infoBoxBorder + ' !important', 'color': cfg.color + ' !important', 'box-shadow': infoBoxShadow + ' !important' },
        ...
        'img': { 'max-width': '100%', 'height': 'auto', 'display': 'block', 'margin': '0 auto', 'filter': isDarkMode ? 'invert(0.85)' : 'none' },
        'figure': { 'margin': '1.5em auto', 'text-align': 'center', 'max-width': '100%', 'display': 'block' },
        'figure img': { 'max-width': '100%', 'width': 'auto', 'height': 'auto', 'max-height': '60vh', 'object-fit': 'contain', 'margin': '0 auto' },
        '.image-with-caption': { 'margin': '1.5em auto', 'text-align': 'center', 'max-width': '100%' },
        '.image-with-caption img': { 'max-width': '100%', 'width': 'auto', 'height': 'auto', 'margin': '0 auto' },
        ...
        '.colophon img': { 'max-width': '150px', 'max-height': '150px', 'height': 'auto', 'filter': isDarkMode ? 'invert(1) hue-rotate(180deg)' : 'none', 'opacity': '1' },
        '.imprint img': { 'max-width': '150px', 'max-height': '150px', 'height': 'auto', 'filter': isDarkMode ? 'invert(1) hue-rotate(180deg)' : 'none', 'opacity': '1' },
        ...
        '.epub-highlight': { 'opacity': isDarkMode ? '0.35' : isCreamMode ? '0.45' : '0.4', 'mix-blend-mode': isDarkMode ? 'screen' : 'multiply' },
      })
        ...
        allContents.forEach(content => {
          try {
            ...
            dynamicStyle.textContent = `
              html { background: ${cfg.bg} !important; }
              body { background-color: ${cfg.bg} !important; color: ${cfg.color} !important; font-size: ${size}px !important; font-family: ${family} !important; -webkit-font-smoothing: antialiased; }
              p, li, span, div, td, th, blockquote, h1, h2, h3, h4, h5, h6 { color: ${cfg.color} !important; }
              p, li, span, div { font-size: inherit !important; }
              a { color: ${isDarkMode ? '#FCD34D' : isCreamMode ? '#7a5c3a' : 'inherit'} !important; }
            `
