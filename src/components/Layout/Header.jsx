// src/components/Layout/Header.jsx — with full light/dark theme support

import { useState, useEffect, useRef } from 'react'
import { Link, useNavigate, useLocation } from 'react-router-dom'
import { Menu, Moon, Search, Sun, X, BookOpen, Film, Newspaper, PenLine, Tag, ChevronDown } from 'lucide-react'
...
const SECONDARY_NAV = [
  { to: '/penulis',  label: 'Penulis',  icon: PenLine },
        ...
        <div className="flex items-center gap-4 py-2.5">
          ...
          <div className="hidden md:flex flex-1 max-w-sm">
            <div className="relative w-full">
              ...
              <input
                ...
                onKeyPress={onKey}
                placeholder="Cari buku, film, penulis..."
          ...
          <nav className="hidden md:flex items-center gap-0.5 flex-shrink-0">
            ...
            <div className="relative" ref={moreRef}>
              ...
              >
                Lainnya <ChevronDown className={`w-3.5 h-3.5 transition-transform ${moreMenuOpen ? 'rotate-180' : ''}`} />
              </button>
          ...
          <div className="hidden md:flex items-center gap-2 ml-auto flex-shrink-0">
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2">
                ...
                >
                  {user?.profilePictureUrl ? (
                    <img key={user.profilePictureUrl} src={user.profilePictureUrl} alt={user.username || 'User'}
                      className="w-6 h-6 rounded-full object-cover" onError={(e) => { e.target.style.display = 'none' }} />
                  ) : (
              ...
              <div className="flex items-center gap-2">
                <Link to="/masuk" className="text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white px-3 py-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-all">
                  Masuk
                </Link>
                <Link to="/daftar" className="text-sm font-semibold bg-amber-500 hover:bg-amber-400 text-gray-950 px-4 py-1.5 rounded-full transition-all hover:scale-105">
                  Daftar
                </Link>
              </div>
            )}
          ...
          <div className="md:hidden flex items-center gap-1.5 ml-auto">
            <button onClick={toggleTheme} className="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all" aria-label="Toggle theme">
              {theme === 'light' ? <Moon className="w-4 h-4" /> : <Sun className="w-4 h-4" />}
            </button>
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all" aria-label="Toggle menu">
        ...
        <div className="md:hidden pb-3">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => setSearchQuery(e.target.value)} onKeyPress={onKey}
              placeholder="Cari buku, film, atau koran..."
            ...
            <div className="border-t border-gray-200 dark:border-gray-800 pt-4">
              {isAuthenticated ? (
                <div className="flex flex-col gap-2">
                  <Link to="/dasbor" className="flex items-center gap-3 p-3 rounded-xl bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-amber-400 dark:hover:border-amber-500 transition-all">
                    {user?.profilePictureUrl ? (
                      <img key={user.profilePictureUrl} src={user.profilePictureUrl} alt={user.username || 'User'}
                        className="w-10 h-10 rounded-full object-cover" onError={(e) => { e.target.style.display = 'none' }} />
                    ) : (
                  ...
                  <button onClick={handleLogout} className="w-full text-sm text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white py-2.5 rounded-xl border border-gray-200 dark:border-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all">
                    Keluar
                  </button>
                </div>
              ) : (
