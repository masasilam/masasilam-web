...
const TYPE_CONFIG = {
  ...
  newspaper: {
    ...
    btnBg: '#7C3AED',
    btnText: '#ffffff',
    link: (item) => sourceHref(item.slug || item.id),
    cta: 'Jelajahi Arsip',
  },
  zine: {
    label: 'Zine & Majalah',
...
const FeaturedBanner = ({ books = [], films = [], articles = [], zines = [] }) => {
  ...
  const featured = [
    ...
    ...articles.slice(0, 1).map(a => ({
      ...a, _type: 'newspaper',
      _image: a.logoUrl || null,
      _title: a.name,
      _sub: a.location || null,
      _desc: a.description,
      _extra: a.totalArticles ? `${a.totalArticles.toLocaleString('id-ID')} Artikel` : null,
    })),
