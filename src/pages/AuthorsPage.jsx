...
const AuthorsPage = () => {
  ...
  const breadcrumbs = [
    { name: 'Beranda', url: '/' },
    { name: 'Daftar Penulis', url: '#' }
  ]

  const structuredData = combineStructuredData(
    generateBreadcrumbStructuredData(breadcrumbs),
    generateCollectionPageStructuredData('authors', authors, currentPage, totalAuthors, LIMIT)
  )

  const pageUrl = currentPage > 1 ? `/penulis?page=${currentPage}` : '/penulis'
  const prevUrl = currentPage > 1 ? (currentPage === 2 ? '/penulis' : `/penulis?page=${currentPage - 1}`) : null
  const nextUrl = currentPage < totalPages ? `/penulis?page=${currentPage + 1}` : null
  ...
  return (
    <>
      <SEO
        title="Daftar Penulis Buku Domain Publik"
        ...
        <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
          ...
          <header className="mb-6 sm:mb-8 pb-4 sm:pb-6 border-b-2 border-primary">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-base">
              Jelajahi {totalAuthors} penulis dan temukan karya-karya terbaik mereka
            </p>
          </header>
