  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <input
        ...
        className="w-14 px-2 py-1.5 text-center text-xs rounded-lg transition-all focus:outline-none border border-stone-200 bg-white text-stone-800 focus:ring-2 focus:ring-amber-400/50 disabled:opacity-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:focus:ring-amber-500/40"
      />
...
const AuthorCard = memo(({ author }) => {
  ...
  return (
    <Link
      ...
      className="group bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-5 border border-stone-200 dark:border-slate-800 hover:border-amber-400 dark:hover:border-amber-500 hover:shadow-lg hover:shadow-amber-100/50 dark:hover:shadow-amber-900/10 transition-all duration-200 flex gap-4 items-center"
    >
      <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl overflow-hidden flex-shrink-0 bg-gradient-to-br from-amber-100 to-amber-200 dark:from-amber-900/30 dark:to-slate-800 border border-amber-200/60 dark:border-amber-800/40 flex items-center justify-center">
        {author.photoUrl ? (
      ...
      <div className="flex-1 min-w-0">
        <h2 className="font-serif text-sm sm:text-base font-bold leading-snug line-clamp-1 mb-1 text-stone-900 dark:text-slate-100 group-hover:text-amber-700 dark:group-hover:text-amber-400 transition-colors capitalize">
          {author.name}
        </h2>
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-1.5 text-xs text-stone-400 dark:text-slate-500">
          {birthYear && (
          ...
          {author.nationality && <span className="flex items-center gap-1">🌍 {author.nationality}</span>}
          {author.birthPlace && (
      ...
      <ArrowRight className="w-4 h-4 flex-shrink-0 self-center text-stone-300 dark:text-slate-600 group-hover:text-amber-500 group-hover:translate-x-0.5 transition-all" />
    </Link>
  )
})
...
const SkeletonCard = memo(() => (
  <div className="animate-pulse bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-5 border border-stone-200 dark:border-slate-800 flex gap-4 items-center" aria-hidden="true">
    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex-shrink-0 bg-stone-200 dark:bg-slate-700" />
...
const AuthorsPage = () => {
  ...
  useEffect(() => {
    const fetchAuthors = async () => {
      setLoading(true)
      try {
        const { data } = await bookService.getAuthors(pageFromUrl, LIMIT, debouncedSearch)
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-stone-50 dark:bg-slate-950 transition-colors duration-300">
        <section className="relative overflow-hidden bg-white dark:bg-slate-950 border-b border-stone-200 dark:border-slate-800 py-10 sm:py-14">
          ...
          <div className="relative container mx-auto px-4 sm:px-6 max-w-5xl">
            <nav className="flex items-center gap-2 text-xs text-stone-400 dark:text-slate-500 mb-5" aria-label="Breadcrumb">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                ...
                <h1 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight text-stone-900 dark:text-white">
                  Daftar Penulis
                </h1>
              ...
              {totalPages > 1 && (
                <div className="hidden sm:flex flex-col items-end px-4 py-2.5 rounded-xl border flex-shrink-0 bg-white border-stone-200 shadow-sm dark:bg-slate-900 dark:border-slate-700 dark:shadow-none">
        ...
        <div className="container mx-auto px-4 sm:px-6 max-w-5xl">
          <div className="mt-6 space-y-3">
            <button
              onClick={() => navigate('/')}
              className="flex items-center gap-2 group text-sm font-medium transition-colors text-stone-500 hover:text-stone-900 dark:text-slate-500 dark:hover:text-slate-100"
            >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none text-stone-400 dark:text-slate-500" />
                <input
                  type="search"
                  value={searchTerm}
                  ...
                  className="w-full pl-10 pr-9 py-2.5 rounded-xl text-sm transition-all focus:outline-none border border-stone-200 bg-white text-stone-900 placeholder-stone-400 shadow-sm focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder-slate-500 dark:shadow-none dark:focus:ring-amber-500/40 dark:focus:border-amber-500/60"
                />
                {searchTerm && (
                  <button
                    onClick={() => setSearchTerm('')}
                    className="absolute right-3 top-1/2 -translate-y-1/2 transition-colors text-stone-400 hover:text-stone-700 dark:text-slate-500 dark:hover:text-slate-300"
              ...
              <select
                ...
                className="px-3 py-2.5 rounded-xl text-sm transition-all focus:outline-none border border-stone-200 bg-white text-stone-800 shadow-sm focus:ring-2 focus:ring-amber-400/40 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:shadow-none dark:focus:ring-amber-500/40 sm:min-w-[170px]"
              >
            ...
            {loading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="font-semibold text-stone-600 dark:text-slate-400 mb-1">
                  {debouncedSearch ? 'Penulis tidak ditemukan' : 'Belum ada penulis'}
                </p>
                {debouncedSearch && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {sorted.map(author => <AuthorCard key={author.id} author={author} />)}
              </div>
            )}
          </section>

          {totalPages > 1 && !loading && (
            <nav className="mt-8 mb-16 flex flex-row flex-wrap items-center justify-center gap-2" aria-label="Navigasi halaman">
              <button
                ...
                {paginationPages.map((p, i) => {
                  ...
                  return (
                    <span key={p} className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => goToPage(p)}
                        className={`w-9 h-9 rounded-lg text-sm font-medium transition-all ${p === pageFromUrl
                          ? 'bg-amber-500 text-white shadow-md shadow-amber-200/80 dark:shadow-amber-900/50'
                          : 'bg-white border border-stone-200 text-stone-600 hover:border-amber-400 hover:text-amber-600 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-400 dark:hover:border-amber-500/70 dark:hover:text-amber-400'
                        }`}
                      >
                        {p}
                      </button>
