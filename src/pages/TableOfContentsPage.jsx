...
/**
 * Helper: Build hierarchical path for chapter navigation
 * @param {Object} chapter - Chapter object with slug
 * @param {Object} parentChapter - Parent chapter object (optional)
 * @returns {string} Full hierarchical path
 */
const buildChapterPath = (chapter, parentChapter = null) => {
  if (!chapter?.slug) return chapter?.chapterNumber || ''
  
  // Level 1 chapters (no parent) - use slug directly
  if (!parentChapter) {
    return chapter.slug
  }
  
  // Level 2+ chapters - combine parent slug with chapter slug
  return `${parentChapter.slug}/${chapter.slug}`
}

// Komponen untuk merender chapter item
  ...
  return (
    <div>
      ...
      <div className="relative">
        {/* Expand/Collapse Button untuk Parent Chapter */}
        {hasSubChapters && (
          <button
            onClick={(e) => {
              e.preventDefault()
      ...
      {/* Sub Chapters (Recursive) - Pass current chapter as parent */}
...
const TableOfContentsPage = () => {
  ...
  // Hitung total main chapters (parentChapterId: null)
