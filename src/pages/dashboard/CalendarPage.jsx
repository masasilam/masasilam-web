...
//
// PERBAIKAN: Field name mismatch — backend Java mengembalikan camelCase,
// bukan snake_case. Semua akses field diupdate:
//   readingData?.minutes_read  → readingData?.minutesRead
//   readingData?.pages_read    → readingData?.pagesRead
//   calendarData?.total_minutes → calendarData?.totalMinutes
//   calendarData?.total_pages   → calendarData?.totalPages
//   calendarData?.active_days   → calendarData?.activeDays
//   selectedDay.minutes_read    → selectedDay.minutesRead
//   selectedDay.pages_read      → selectedDay.pagesRead
//   book.title (CalendarBookEntry) → book.title (tidak berubah)
//
// Juga: normalize() → ambil res.data bukan res langsung
// ============================================
...
const CalendarPage = () => {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute('content', 'Lacak aktivitas membaca harian Anda dengan kalender interaktif')
    loadCalendarData()
  }, [year, month])
  ...
  const loadCalendarData = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const res = await dashboardService.getCalendar(year, month + 1)
  ...
  // PERBAIKAN: baca minutesRead (camelCase), bukan minutes_read
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6">
                ...
                {readingData && (
                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block group-focus:block z-10 pointer-events-none">
                    <div className="bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap">
                      {/* PERBAIKAN: minutesRead + pagesRead */}
                      {readingData.minutesRead} menit · {readingData.pagesRead} halaman
                    </div>
                  </div>
                )}
              </button>
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-4 mt-4 sm:mt-6 pt-4 sm:pt-6 border-t dark:border-gray-700">
          ...
          <div className="flex gap-1" role="img" aria-label="Legenda intensitas membaca">
            <div className="w-4 h-4 sm:w-6 sm:h-6 rounded bg-gray-100 dark:bg-gray-800" />
            <div className="w-4 h-4 sm:w-6 sm:h-6 rounded bg-green-200 dark:bg-green-900" />
            <div className="w-4 h-4 sm:w-6 sm:h-6 rounded bg-green-400 dark:bg-green-700" />
            <div className="w-4 h-4 sm:w-6 sm:h-6 rounded bg-green-600 dark:bg-green-600" />
            <div className="w-4 h-4 sm:w-6 sm:h-6 rounded bg-green-800 dark:bg-green-500" />
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              {/* PERBAIKAN: totalMinutes (camelCase) */}
              <p className="text-xl sm:text-2xl font-bold">{calendarData?.totalMinutes || 0} menit</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              {/* PERBAIKAN: totalPages (camelCase) */}
              <p className="text-xl sm:text-2xl font-bold">{calendarData?.totalPages || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6 sm:col-span-2 lg:col-span-1">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              {/* PERBAIKAN: activeDays (camelCase) */}
              <p className="text-xl sm:text-2xl font-bold">{calendarData?.activeDays || 0} hari</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {selectedDay && (
        <div
          className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6 relative"
          role="dialog"
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              {/* PERBAIKAN: minutesRead */}
              <p className="text-base sm:text-lg font-semibold">{selectedDay.minutesRead} menit</p>
            </div>
            <div>
              <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Halaman Dibaca</p>
              {/* PERBAIKAN: pagesRead */}
              <p className="text-base sm:text-lg font-semibold">{selectedDay.pagesRead} halaman</p>
            </div>
          </div>
              ...
              <ul className="space-y-2">
                {selectedDay.books.map((book, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm sm:text-base">
                    ...
                    {/* CalendarBookEntry.title tidak berubah */}
