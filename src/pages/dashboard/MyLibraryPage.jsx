...
const WOOD_THEMES = {
  dark: {
    panelBg:
      'repeating-linear-gradient(90deg,transparent 0,transparent 18px,rgba(0,0,0,0.04) 18px,rgba(0,0,0,0.04) 20px),linear-gradient(180deg,#2e1f0a 0%,#3d2a10 25%,#4a3418 55%,#402e14 80%,#3a2810 100%)',
    borderImage: 'linear-gradient(180deg,#6b4c1e,#3d2508,#7a5828) 1',
    shelfBar: 'linear-gradient(180deg,#b07830 0%,#8a5c20 20%,#a87030 45%,#8B5E2A 70%,#6b4418 100%)',
    shelfBorderTop: '#e0b040',
    shelfShadowTop: 'linear-gradient(180deg,rgba(255,200,70,0.4),transparent)',
    shelfShadowBottom: 'linear-gradient(180deg,transparent,rgba(0,0,0,0.45))',
    rowShadow: 'linear-gradient(180deg,rgba(0,0,0,0.55),rgba(0,0,0,0.1),transparent)',
    bodyBg: 'linear-gradient(180deg,#1a1208 0%,#221810 50%,#1a120a 100%)',
    skeletonPiece: 'rgba(255,255,255,0.07)',
    emptyIconBg: 'rgba(255,255,255,0.06)',
    emptyTitle: 'text-amber-200/50',
    emptySubtitle: 'text-amber-200/30',
  },
  light: {
    panelBg:
      'repeating-linear-gradient(90deg,transparent 0,transparent 18px,rgba(0,0,0,0.03) 18px,rgba(0,0,0,0.03) 20px),linear-gradient(180deg,#d9b878 0%,#c9a463 25%,#b8925a 55%,#a67f4a 80%,#96713f 100%)',
    borderImage: 'linear-gradient(180deg,#e8c68a,#a67f4a,#8a6535) 1',
    shelfBar: 'linear-gradient(180deg,#f0cf8a 0%,#dcae5c 20%,#e0b666 45%,#c99748 70%,#a97a38 100%)',
    shelfBorderTop: '#fde3a7',
    shelfShadowTop: 'linear-gradient(180deg,rgba(255,240,200,0.5),transparent)',
    shelfShadowBottom: 'linear-gradient(180deg,transparent,rgba(70,45,10,0.3))',
    rowShadow: 'linear-gradient(180deg,rgba(60,40,10,0.3),rgba(60,40,10,0.06),transparent)',
    bodyBg: 'linear-gradient(180deg,#ecd9b0 0%,#e0c898 50%,#d2b380 100%)',
    skeletonPiece: 'rgba(60,40,10,0.1)',
    emptyIconBg: 'rgba(60,40,10,0.06)',
...
const BookTooltip = ({ book, anchorRef, pct, badgeColor, statusLabel }) => {
  ...
  return createPortal(
    <div style={{ position: 'absolute', top: pos.top, left: pos.left, width: pos.width, transform: 'translateY(-100%)', zIndex: 99999, pointerEvents: 'none' }}>
      <div
        style={{
          background: bg,
          ...
          borderRadius: 16,
          padding: 14,
          boxShadow: isDark ? '0 20px 60px rgba(0,0,0,0.7),0 0 0 1px rgba(255,255,255,0.04)' : '0 20px 60px rgba(0,0,0,0.15),0 0 0 1px rgba(0,0,0,0.04)',
        }}
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            ...
            <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 700, color: pct >= 100 ? '#4ade80' : '#f59e0b' }}>
              {pct}%
            </span>
          </div>
        </div>
...
const BookSpine = ({ book, index, scale }) => {
  const navigate = useNavigate()
  const hasRealSpine = !!book?.spineCoverUrl
  ...
  const fallbackWidth = Math.round(SPINE_WIDTHS[index % SPINE_WIDTHS.length] * scale)
  ...
  const showRealSpine = hasRealSpine && !imgError
      ...
      >
        <div
          style={{
            ...
            background: showRealSpine ? 'transparent' : colors ? `linear-gradient(150deg,${light} 0%,${bg} 45%,${dark} 100%)` : '#2a1c0a',
            boxShadow: hovered
...
const Shelf = ({ books, startIndex, scale, isDark }) => {
  const theme = isDark ? WOOD_THEMES.dark : WOOD_THEMES.light
  const rowHeight = Math.round(Math.max(SPINE_MAX_HEIGHT, ...SPINE_HEIGHTS) * scale) + 25
    ...
    <div>
      <div
        style={{
          ...
          borderImage: theme.borderImage,
        }}
      >
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: SPINE_GAP, minHeight: rowHeight, paddingBottom: 6 }}>
          {books.map((book, i) => (
            <BookSpine key={book?.bookId || i} book={book} index={startIndex + i} scale={scale} />
          ))}
        </div>
        <div
          style={{
            height: 24,
            margin: '0 -18px',
            background: theme.shelfBar,
            borderTop: `2px solid ${theme.shelfBorderTop}`,
            boxShadow: '0 6px 18px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,215,80,0.3),inset 0 -2px 4px rgba(0,0,0,0.35)',
            position: 'relative',
          }}
        >
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: theme.shelfShadowTop }} />
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 10, background: theme.shelfShadowBottom }} />
        </div>
      </div>
      <div style={{ height: 14, background: theme.rowShadow }} />
    </div>
  )
}

const SkeletonShelf = ({ count, scale, isDark }) => {
  const theme = isDark ? WOOD_THEMES.dark : WOOD_THEMES.light
  const rowHeight = Math.round(Math.max(SPINE_MAX_HEIGHT, ...SPINE_HEIGHTS) * scale) + 25

  return (
    <div>
      <div style={{ padding: '20px 18px 0', background: theme.panelBg, borderLeft: '16px solid', borderRight: '16px solid', borderImage: theme.borderImage }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: SPINE_GAP, minHeight: rowHeight, paddingBottom: 6 }}>
          {Array.from({ length: count }).map((_, i) => (
            <div
              ...
              style={{
                flexShrink: 0,
                width: Math.round(SPINE_WIDTHS[i % SPINE_WIDTHS.length] * scale),
                height: Math.round(SPINE_HEIGHTS[i % SPINE_HEIGHTS.length] * scale),
                borderRadius: '1px 4px 4px 1px',
                background: theme.skeletonPiece,
              }}
            />
          ))}
        </div>
        <div style={{ height: 24, margin: '0 -18px', background: theme.shelfBar, borderTop: `2px solid ${theme.shelfBorderTop}`, boxShadow: '0 6px 18px rgba(0,0,0,0.5)' }} />
      </div>
      <div style={{ height: 14, background: theme.rowShadow }} />
    </div>
  )
...
const MyLibraryPage = () => {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      const nextScale = getScaleForWidth(window.innerWidth)
      setScale(nextScale)
      setBooksPerShelf(Math.max(4, Math.floor(innerW / (AVG_SPINE_W * nextScale + SPINE_GAP))))
    }
    calc()
  ...
  const BOOKS_PER_PAGE = booksPerShelf * 2
  ...
  useEffect(() => {
    document.title = 'Perpustakaan Saya - Dashboard'
  }, [])
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl xs:text-2xl sm:text-3xl font-bold text-stone-900 dark:text-slate-50">Perpustakaan Saya</h1>
          </div>
          <p className="text-sm text-stone-500 dark:text-slate-400 italic">Semua buku yang pernah Anda baca</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        {[
          ...
          <div key={s.label} className="bg-white dark:bg-slate-900 rounded-2xl border border-stone-200 dark:border-slate-700 py-3 px-2 sm:px-4 text-center">
            <p className={`text-xl sm:text-2xl font-bold ${s.color}`}>{s.value}</p>
            <p className="text-[10px] sm:text-xs text-stone-400 dark:text-slate-500 uppercase tracking-wider mt-0.5">{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {FILTER_OPTIONS.map((o) => (
          <button
            ...
            className={`px-3.5 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-medium transition-all border ${
              filter === o.value
          ...
          >
            {o.label}
            {filter === o.value && totalData > 0 && <span className="ml-1.5 text-[11px] bg-stone-950/15 rounded-full px-1.5 py-0.5">{totalData}</span>}
          </button>
        ))}
      </div>
      ...
      {error && error !== 'auth' && (
        <div className="flex items-center justify-between gap-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-4">
          ...
          <button onClick={fetchLibrary} className="flex items-center gap-1.5 text-sm font-semibold px-4 py-1.5 bg-red-500 hover:bg-red-600 text-white rounded-xl transition-colors">
      ...
      <div ref={shelfRef} className="overflow-hidden rounded-2xl border border-stone-200 dark:border-slate-700 shadow-lg">
        <div className="px-4 sm:px-5 py-3 border-b border-stone-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between gap-2">
          <p className="text-[10px] sm:text-xs font-black uppercase tracking-[0.18em] sm:tracking-[0.22em] text-stone-400 dark:text-slate-500">— Koleksi Anda —</p>
          {!loading && books.length > 0 && (
            <p className="text-[10px] sm:text-xs text-stone-400 dark:text-slate-500 flex-shrink-0">
              {totalPages > 1 ? `Hal. ${page} / ${totalPages}` : `${books.length} buku ditampilkan`}
            </p>
          )}
        </div>

        <div className="p-3 sm:p-4 md:p-5" style={{ background: theme.bodyBg }}>
          {loading ? (
            <div className="space-y-2">
              <SkeletonShelf count={booksPerShelf} scale={scale} isDark={isDark} />
              <SkeletonShelf count={Math.floor(booksPerShelf * 0.65)} scale={scale} isDark={isDark} />
            </div>
          ) : books.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 sm:py-20 text-center">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4" style={{ background: theme.emptyIconBg }}>
                <BookOpen className="w-8 h-8 text-amber-400 opacity-40" />
              </div>
              <p className={`text-sm italic mb-1 ${theme.emptyTitle}`}>Rak kosong...</p>
              <p className={`text-xs ${theme.emptySubtitle}`}>Mulai membaca untuk mengisi rak ini</p>
            </div>
          ) : (
            <div className="space-y-2">
              {shelves.map((row, ri) => (
                <Shelf key={ri} books={row} startIndex={ri * booksPerShelf} scale={scale} isDark={isDark} />
              ))}
            </div>
          )}
        </div>
      ...
      {totalPages > 1 && !loading && (
        <nav className="flex items-center justify-center gap-1.5 sm:gap-2" aria-label="Pagination">
          <button
            ...
            disabled={page === 1}
            className="px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium rounded-xl border border-stone-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-stone-600 dark:text-slate-400 disabled:opacity-40 disabled:cursor-not-allowed hover:border-amber-400 hover:text-amber-600 dark:hover:border-amber-500 transition-all"
          >
            ← Sebelumnya
          </button>
              ...
              return (
                <button
                  ...
                  className={`w-8 h-8 sm:w-9 sm:h-9 text-xs sm:text-sm font-medium rounded-xl transition-all ${
                    p === page ? 'bg-amber-500 text-stone-950 font-bold shadow-sm' : 'text-stone-500 dark:text-slate-400 hover:bg-stone-100 dark:hover:bg-slate-800'
                  }`}
                >
                  {p}
                </button>
