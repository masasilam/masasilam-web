...
const GoalFormModal = ({ editingGoal, onClose, onSaved }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingGoal) {
        await dashboardService.updateGoal(editingGoal.id, payload)
      } else {
        await dashboardService.createGoal(payload)
      }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 id="modal-title" className="font-bold text-lg text-gray-900 dark:text-gray-100">
            {editingGoal ? 'Edit Target' : 'Tambah Target Baru'}
          </h2>
          <button
            onClick={onClose}
            className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors rounded focus:outline-none focus:ring-2 focus:ring-primary"
            aria-label="Tutup"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 text-sm rounded-lg border bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary transition ${
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 text-sm rounded-lg border bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary transition ${
                errors.targetValue ? 'border-red-400' : 'border-gray-300 dark:border-gray-600'
              }`}
            />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 text-sm rounded-lg border bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary transition ${
                  errors.startDate ? 'border-red-400' : 'border-gray-300 dark:border-gray-600'
                }`}
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 text-sm rounded-lg border bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary transition ${
                  errors.endDate ? 'border-red-400' : 'border-gray-300 dark:border-gray-600'
                }`}
              />
          ...
          <div className="flex gap-3 pt-1">
            <button
              onClick={onClose}
              disabled={saving}
              className="flex-1 px-4 py-2.5 text-sm bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-gray-400"
            >
              Batal
            </button>
            <button
              ...
              disabled={saving}
              className="flex-1 px-4 py-2.5 text-sm bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-primary flex items-center justify-center gap-2"
            >
              {saving ? (
                <>
...
const DeleteConfirmModal = ({ goal, onClose, onDeleted }) => {
  ...
  const handleDelete = async () => {
    try {
      setDeleting(true)
      await dashboardService.deleteGoal(goal.id)
      onDeleted()
    } catch (err) {
...
const GoalCard = ({ goal, onEdit, onDelete }) => {
  ...
  const barColor =
    ...
    progress >= 50  ? 'bg-yellow-500' : 'bg-primary'
    ...
    <article className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 hover:shadow-xl transition-shadow">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-base truncate text-gray-900 dark:text-gray-100">
              {goal.title}
            </h3>
            {goal.description && (
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button
            onClick={() => onEdit(goal)}
            className="p-2 text-gray-500 hover:text-primary transition-colors rounded focus:outline-none focus:ring-2 focus:ring-primary"
          ...
          <button
            ...
            className="p-2 text-gray-500 hover:text-red-500 transition-colors rounded focus:outline-none focus:ring-2 focus:ring-red-500"
      ...
      <div className="mb-3">
        ...
        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2" role="progressbar"
          aria-valuenow={Math.min(progress, 100)} aria-valuemin="0" aria-valuemax="100">
          <div
...
const GoalsPage = () => {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute('content', 'Tetapkan dan lacak target membaca Anda')
  }, [])
  ...
  if (error) {
    return (
      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6 text-center" role="alert">
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {summaryItems.map(({ label, value, icon: Icon, bg, color, colSpan }) => (
          <div key={label}
            className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-5 ${colSpan ? 'col-span-2 lg:col-span-1' : ''}`}>
      ...
      <div className="space-y-3">
        ...
        {goals?.active && goals.active.length > 0 ? (
          ...
          <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl border border-dashed border-gray-300 dark:border-gray-600">
            ...
            <button
              onClick={() => setShowAddModal(true)}
              className="px-5 py-2 bg-primary text-white text-sm rounded-lg hover:bg-primary-dark transition-colors focus:outline-none focus:ring-2 focus:ring-primary"
            >
              Buat Target Pertama
            </button>
          </div>
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {goals.completed.map(goal => {
              ...
              return (
                <div key={goal.id}
                  className="bg-white dark:bg-gray-800 rounded-xl shadow p-4 border-2 border-green-500">
