  ...
  return (
    <div className="space-y-1.5 mt-2">
      <div className="p-2.5 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800 text-sm font-mono">
        ...
        <span className="bg-red-200 dark:bg-red-800 text-red-800 dark:text-red-200 px-1 rounded font-semibold">{originalText}</span>
      ...
      <div className="p-2.5 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800 text-sm font-mono">
        <span className="text-gray-500 dark:text-gray-400 text-xs mr-1">+</span>
        <span className="text-gray-500">{before}</span>
        <span className="bg-green-200 dark:bg-green-800 text-green-800 dark:text-green-200 px-1 rounded font-semibold">{correctedText}</span>
        <span className="text-gray-500">{after}</span>
      </div>
    </div>
  )
    ...
    <article className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-3 flex-wrap text-xs text-gray-500 dark:text-gray-400">
            <span className="flex items-center gap-1"><User size={11} />{correction.submittedByUsername}</span>
            <span className="flex items-center gap-1"><Clock size={11} />{formatDate(correction.createdAt)}</span>
      ...
      {correction.reviewNote && (
        <div className="mt-3 p-2.5 bg-gray-50 dark:bg-gray-700/50 rounded-lg text-xs text-gray-600 dark:text-gray-400">
          <span className="font-medium">Catatan admin: </span>{correction.reviewNote}
        </div>
      )}
          ...
          {!showRejectInput ? (
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition disabled:opacity-60"
              >
              ...
              <button
                ...
                disabled={isProcessing}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-700 hover:bg-red-50 dark:hover:bg-red-900/20 border border-gray-300 dark:border-gray-600 hover:border-red-300 dark:hover:border-red-700 text-gray-700 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-400 text-sm font-medium rounded-lg transition disabled:opacity-60"
              >
            ...
            <div className="space-y-2">
              <input
                ...
                className="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-400"
                autoFocus
              />
              <div className="flex gap-2">
                <button onClick={() => { setShowRejectInput(false); setRejectNote('') }}
                  className="flex-1 px-4 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                  Batal
                </button>
                <button onClick={handleReject} disabled={isProcessing}
                  className="flex-1 px-4 py-2 text-sm rounded-lg bg-red-600 hover:bg-red-700 text-white font-medium transition disabled:opacity-60 flex items-center justify-center gap-2">
  ...
  return (
    <div className="flex items-center justify-center gap-2 mt-6">
      <button onClick={() => onPageChange(page - 1)} disabled={page <= 1}
        className="p-2 rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition disabled:opacity-40">
        <ChevronLeft size={16} />
      </button>
...
const CorrectionQueuePage = () => {
  ...
  const handleApprove = useCallback(async (correctionId) => {
    ...
    try {
      ...
      showFeedback('success', '✓ Koreksi disetujui.')
      await fetchCorrections(activeTab, page)
    } catch (err) {
      showFeedback('error', '✗ Gagal menyetujui: ' + (err?.response?.data?.message || err.message))
    } finally {
  ...
  const handleReject = useCallback(async (correctionId, note) => {
    ...
    try {
      ...
      showFeedback('error', '✗ Gagal menolak: ' + (err?.response?.data?.message || err.message))
    } finally {
      setProcessingId(null)
    }
  }, [activeTab, page, fetchCorrections])
  ...
  const tabs = [
    { id: 'PENDING',  label: isAdmin ? 'Menunggu Review' : 'Menunggu' },
    { id: 'APPROVED', label: isAdmin ? 'Disetujui' : 'Riwayat Perbaikan' },
  ...
  return (
    <DashboardShell
      loading={loading}
      error={error}
      onRetry={() => fetchCorrections(activeTab, page)}
      onLogin={() => navigate('/masuk', { state: { from: '/dasbor/koreksi' } })}
    >
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm">
            {isAdmin
              ? 'Review laporan typo dari pembaca.'
            ...
            {corrections.length === 0 ? (
              <div className="text-center py-16 text-gray-400">
                ...
                <p className="text-sm mt-1">
                  {activeTab === 'PENDING'
                    ? isAdmin ? 'Semua laporan sudah diproses!' : 'Belum ada laporan menunggu.'
