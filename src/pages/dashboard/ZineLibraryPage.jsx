...
const FALLBACK_COLORS = [
  { bg: '#065f46', light: '#10b981', dark: '#022c22', text: 'rgba(255,255,255,0.92)' },
  { bg: '#047857', light: '#34d399', dark: '#014737', text: 'rgba(255,255,255,0.92)' },
  { bg: '#0f766e', light: '#2dd4bf', dark: '#042f2e', text: 'rgba(255,255,255,0.92)' },
  { bg: '#15803d', light: '#4ade80', dark: '#052e16', text: 'rgba(255,255,255,0.92)' },
  { bg: '#0e7490', light: '#22d3ee', dark: '#083344', text: 'rgba(255,255,255,0.92)' },
  { bg: '#1d4ed8', light: '#60a5fa', dark: '#1e1b4b', text: 'rgba(255,255,255,0.92)' },
  { bg: '#6d28d9', light: '#a78bfa', dark: '#2e1065', text: 'rgba(255,255,255,0.92)' },
  { bg: '#be185d', light: '#f472b6', dark: '#500724', text: 'rgba(255,255,255,0.92)' },
]

const SPINE_HEIGHTS = [148, 132, 156, 122, 152, 136, 142, 126, 160, 134, 150, 124]
const SPINE_WIDTHS  = [30,  34,  28,  36,  32,  30,  34,  28,  32,  36,  30,  34]

function useZineColor(coverUrl, fallbackIndex) {
...
// ── Zine Spine Tooltip ────────────────────────────────────────────────────────
const ZineTooltip = ({ zine, anchorRef, pct, badgeColor, statusLabel }) => {
  ...
  return createPortal(
    <div style={{
      position:'absolute', top:pos.top, left:pos.left, width:210,
      ...
      {/* Stripe on top of tooltip cover */}
      {zine?.coverImageUrl && (
        <div style={{ width:'100%', aspectRatio:'2/3', marginBottom:8, borderRadius:4, overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,0.25)', position:'relative' }}>
          <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:'linear-gradient(90deg,#10b981,#2dd4bf)', zIndex:1 }} />
      ...
      {zine?.genre && <p style={{ fontSize:10, color:'#10b981', margin:'0 0 5px', letterSpacing:0.5 }}>{zine.genre}</p>}
      ...
      <div>
        ...
        <div style={{ height:3, background:barBg, borderRadius:2, overflow:'hidden' }}>
          <div style={{ height:'100%', width:`${pct}%`, background:'linear-gradient(90deg,#059669,#34d399)', borderRadius:2 }} />
        </div>
      </div>
      {(zine?.bookmarkCount > 0 || zine?.highlightCount > 0) && (
        <div style={{ display:'flex', gap:8, marginTop:6 }}>
          {zine.bookmarkCount  > 0 && <span style={{ fontSize:10, color:subtitleClr }}>📑 {zine.bookmarkCount}</span>}
          {zine.highlightCount > 0 && <span style={{ fontSize:10, color:subtitleClr }}>✨ {zine.highlightCount}</span>}
        </div>
      )}
    </div>,
    document.body
...
// ── Zine Spine ────────────────────────────────────────────────────────────────
const ZineSpine = ({ zine, index }) => {
  const navigate = useNavigate()
  const colors   = useZineColor(zine?.coverImageUrl, index)
  const [hovered, setHovered] = useState(false)
  const spineRef = useRef(null)

  const height = SPINE_HEIGHTS[index % SPINE_HEIGHTS.length]
  const width  = SPINE_WIDTHS[index  % SPINE_WIDTHS.length]
  const pct    = Math.round(zine?.progressPercentage || 0)

  const bg        = colors?.bg   ?? '#065f46'
  const light     = colors?.light ?? '#10b981'
  const dark      = colors?.dark  ?? '#022c22'
  const textColor = colors?.text  ?? 'rgba(255,255,255,0.92)'

  const badgeColor  = zine?.readingStatus === 'completed' ? '#34d399' : zine?.readingStatus === 'reading' ? '#4a9eff' : 'rgba(255,255,255,0.25)'
  const statusLabel = zine?.readingStatus === 'completed' ? 'Selesai' : zine?.readingStatus === 'reading' ? 'Sedang Dibaca' : 'Belum Dibaca'

  const handleClick = useCallback(() => {
    if (!zine?.zineSlug) return
    navigate(`/zine/${zine.zineSlug}/baca`, { state: zine.lastCfi ? { lastCfi: zine.lastCfi } : {} })
  }, [navigate, zine?.zineSlug, zine?.lastCfi])
  ...
  return (
    <>
      <div ref={spineRef} onClick={handleClick}
        ...
        aria-label={zine?.zineTitle} role="link" tabIndex={0}
        onKeyDown={e => e.key === 'Enter' && handleClick()}>
        <div style={{
          height, width, borderRadius:'1px 3px 3px 1px',
          background: colors ? `linear-gradient(150deg, ${light} 0%, ${bg} 50%, ${dark} 100%)` : '#022c22',
          ...
          overflow:'hidden', transition:'transform 0.2s ease, filter 0.2s ease',
      ...
      {hovered && <ZineTooltip zine={zine} anchorRef={spineRef} pct={pct} badgeColor={badgeColor} statusLabel={statusLabel} />}
    </>
  )
}

// ── Shelf ─────────────────────────────────────────────────────────────────────
const Shelf = ({ zines, startIndex }) => (
  <div>
    <div style={{
      padding:'18px 16px 0',
      background:`repeating-linear-gradient(90deg,transparent 0,transparent 18px,rgba(0,0,0,0.035) 18px,rgba(0,0,0,0.035) 20px),linear-gradient(180deg,#0d3321 0%,#14402a 30%,#1a5235 60%,#0d3321 100%)`,
      borderLeft:'14px solid', borderRight:'14px solid',
      borderImage:'linear-gradient(180deg,#1a6040,#0a2e18,#1e7048) 1',
    }}>
      <div style={{ display:'flex', alignItems:'flex-end', gap:3, minHeight:170, paddingBottom:6 }}>
        {zines.map((zine, i) => <ZineSpine key={zine?.zineId || i} zine={zine} index={startIndex + i} />)}
      </div>
      <div style={{
        height:22, margin:'0 -16px',
        background:'linear-gradient(180deg,#1a7a40 0%,#145a30 25%,#1e8a48 50%,#165835 75%,#0d4020 100%)',
        borderTop:'2px solid #2dd4bf', boxShadow:'0 5px 14px rgba(0,0,0,0.65), inset 0 1px 0 rgba(52,211,153,0.25)',
        position:'relative',
      }}>
        <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:'linear-gradient(180deg,rgba(52,211,153,0.35),transparent)' }} />
...
const ZineLibraryPage = () => {
  ...
  const ZINES_PER_PAGE  = 16
  const ZINES_PER_SHELF = 8
  ...
  useEffect(() => { document.title = 'Perpustakaan Zine — Dashboard' }, [])

  const fetchLibrary = useCallback(async () => {
    try {
      setLoading(true); setError(null)
      const response = await zineDashboardService.getZineLibrary(filter, page, ZINES_PER_PAGE, sortBy)
      if (response?.data) {
        setZines(response.data.items || [])
        const total = response.data.totalData || response.data.total || 0
        setTotalData(total)
        setTotalPages(Math.max(1, Math.ceil(total / ZINES_PER_PAGE)))
      } else { setZines([]); setTotalData(0); setTotalPages(1); setError('network') }
    } catch (err) {
      if (err?.response?.status === 401) setError('auth')
      else setError('network')
      setZines([]); setTotalData(0); setTotalPages(1)
  ...
  const shelves = useMemo(() => {
    const rows = []
    for (let i = 0; i < zines.length; i += ZINES_PER_SHELF)
      rows.push(zines.slice(i, i + ZINES_PER_SHELF))
    return rows
  }, [zines])

  const countReading   = zines.filter(z => z?.readingStatus === 'reading').length
  const countCompleted = zines.filter(z => z?.readingStatus === 'completed').length
  ...
  if (error === 'auth') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 px-4 text-center">
        ...
        <button onClick={() => navigate('/masuk', { state: { from: '/dasbor/zine' } })}
          className="px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold rounded-lg transition">
          Masuk Kembali
        </button>
      </div>
    )
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 font-serif">
      ...
      <div className="mb-6 sm:mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 italic text-sm">Koleksi zine &amp; majalah yang pernah Anda baca</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <select value={sortBy} onChange={e => handleSortChange(e.target.value)}
            className="bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-md px-3 py-1.5 text-sm cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-500">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 mb-6 flex flex-wrap gap-3 items-center">
        ...
        {filterOptions.map(o => (
          <button key={o.value} onClick={() => handleFilterChange(o.value)}
            className={`px-4 py-1.5 rounded-full text-sm transition-colors border focus:outline-none focus:ring-2 focus:ring-emerald-500 ${
              filter === o.value
                ? 'bg-emerald-500 text-white border-emerald-500'
                : 'bg-transparent text-gray-600 dark:text-gray-400 border-gray-300 dark:border-gray-600 hover:border-emerald-500 hover:text-emerald-600'
            }`}>
            {o.label}
          </button>
        ))}
      ...
      {error && error !== 'auth' && (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-6">
          <p className="text-red-700 dark:text-red-400 mb-2">Gagal memuat data perpustakaan zine</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400 dark:text-gray-500">
            ...
            <p className="italic text-base">Belum ada zine di perpustakaan</p>
            <button onClick={() => navigate('/zine')}
              className="mt-4 px-5 py-2.5 rounded-xl text-sm font-semibold bg-emerald-500 hover:bg-emerald-400 text-white transition-all">
              Jelajahi Zine
            </button>
          </div>
        ) : (
          <div className="p-4 sm:p-6 space-y-2">
            {shelves.map((row, ri) => <Shelf key={ri} zines={row} startIndex={ri * ZINES_PER_SHELF} />)}
          </div>
        )}
      </div>
...
export default ZineLibraryPage